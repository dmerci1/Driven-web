...
import { Segment, Image, Header, Divider, Button, Grid, Card, Icon } from 'semantic-ui-react';
import Dropzone from 'react-dropzone';
import Cropper from 'react-cropper';
import 'cropperjs/dist/cropper.css';
import { toastr } from 'react-redux-toastr';
import { uploadProfileImage, deleteImage, setMainPhoto } from '../../actions/ImageActions';
...
uploadImage = async () => {
  try {
    ...
    toastr.error('OOOOOOOOOOOP!, error.message')
  }
}

handlePhotoDelete = (photo) => () => {
  try {
    this.props.deleteImage(photo);
  } catch (error) {
    toastr.error('Fuckkk', error.message)
  }
}
      ...
      <Segment>
        ...
        <Grid>
          <Grid.Row />
            <Grid.Column width={4}>
              ...
              <Dropzone onDrop={this.onDrop} multiple={false}>
                <div style={{paddingTop: '30px', textAlign: 'center'}}>
                  ...
                  <Header content='Drop image here of click add' />
                </div>
              </Dropzone>
            </Grid.Column>
            <Grid.Column width={1} />
            <Grid.Column width={4}>
              <Header color="teal" sub content="Resize Image" />
              {this.state.files[0] &&
              <Cropper
            ...
            <Grid.Column width={1} />
            <Grid.Column width={4}>
              <Header color="teal" sub content="Preview and Upload" />
              {this.state.files[0] && (
                <div>
                  <Image
          ...
          <Card.Group itemsPerRow={5}>
            <Card>
              <Image src={profile.photoURL || '../../assets/pat.jpg'}/>
